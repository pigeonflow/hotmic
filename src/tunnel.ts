...
const BIN_DIR = join(homedir(), ".hotmic", "bin");
...
function getDownloadURL(): string {
  ...
  // Linux
  if (a === "arm64" || (a as string) === "aarch64") return `${base}/cloudflared-linux-arm64`;
...
export function startTunnel(port: number): Promise<string> {
  return new Promise(async (resolve, reject) => {
    ...
    const handleOutput = (data: Buffer) => {
      ...
      if (match && !resolved && !match[0].includes("api.trycloudflare.com")) {
        resolved = true;
